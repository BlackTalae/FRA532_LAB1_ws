#!/usr/bin/python3
"""
ICP-EKF SLAM Odometry Node

Combines Extended Kalman Filter (EKF) odometry with ICP scan matching
to perform SLAM (Simultaneous Localization and Mapping).

Features:
- EKF for odometry prediction using wheel encoders and IMU
- ICP for scan matching and pose correction
- Keyframe-based mapping
- Occupancy grid generation
- IMU calibration on startup
- Jump detection to prevent unrealistic pose changes
"""
import signal
import sys
...
# Import custom scan processing and ICP functions
from fra532_lab1_package.scan import *
from fra532_lab1_package.icp import *
...
class ICPNode(Node):
    def __init__(self):
        ...
        # --- Estimator & SLAM --- 
        # Initialize Extended Kalman Filter with noise parameters
        process_noise = [0.5, 0.5, np.deg2rad(0.1)]  # [x_std, y_std, yaw_std]
        measurement_noise = [np.deg2rad(0.1)]  # [yaw_std] from IMU
        self.ekf = ExtendedKalmanFilter(process_noise, measurement_noise)
        
        # EKF pose estimate [x, y, theta]
        ...
        self.L = 0.16   # Wheelbase (meters)
        self.last_joint_pos = None
        self.last_time = None
        
        # --- ICP SLAM Position ---
        self.icp_pose = np.array([0.0, 0.0, 0.0])  # Point-to-Plane ICP pose
        ...
        self.icp_path_msg = Path()  # Point-to-Plane ICP
        self.icp_path_msg.header.frame_id = self.path_base_frame
        self.icp_p2point_path_msg = Path()  # Point-to-Point ICP
        self.icp_p2point_path_msg.header.frame_id = self.path_base_frame
    ...
    def laser_scan_callback(self, msg):
        ...
        if self.enable_icp and len(self.keyframes) > 0:
            ...
            if local_map.shape[0] >= self.min_local_map_points:
                # ========================================
                # Run BOTH ICP variants for comparison
                # ========================================
                
                # 1) Point-to-Point ICP (original)
                x_p2point, y_p2point, theta_p2point, count_p2point, error_p2point, success_p2point = icp_matching_point2point(
                ...
                # 2) Point-to-Plane ICP (improved)
                x, y, theta, count, error, success = icp_matching_point2plane(
                    previous_points=local_map,
                    current_points=self.current_scan,
                    init_x=predicted_icp_pose[0],
                    init_y=predicted_icp_pose[1],
                    ...
                    ERROR_BOUND=0.0005,
                    max_corr_dist=0.1,
                    normal_k=5  # Number of neighbors for normal estimation
                )
                
                # Calculate corrections for point-to-plane (main)
                ...
                # Calculate corrections for point-to-point (comparison)
                correction_dist_p2point = np.sqrt((x_p2point - predicted_icp_pose[0])**2 + 
                                                 (y_p2point - predicted_icp_pose[1])**2)
                correction_angle_p2point = abs(wrap(theta_p2point - predicted_icp_pose[2]))
                
                # Log periodically (compare both methods)
                if self.total_scans % 10 == 0:
                    self.get_logger().info(
                        f'ICP P2Plane: err={error:.4f}, iter={count}/40, corr={correction_dist:.3f}m/{np.rad2deg(correction_angle):.1f}°'
                    )
                    self.get_logger().info(
                        f'ICP P2Point: err={error_p2point:.4f}, iter={count_p2point}/40, corr={correction_dist_p2point:.3f}m/{np.rad2deg(correction_angle_p2point):.1f}°'
                    )
                
                # ========================================
                # Process Point-to-Plane ICP (main path)
                # ========================================
                # Validate ICP result
                if success and error < 0.15:
                    ...
                    if correction_dist < 0.15 and correction_angle < np.deg2rad(20):
                        ...
                        if not self.detect_jump(candidate_pose, delta_ekf):
                            ...
                            self.get_logger().error('ICP P2Plane result would cause JUMP - REJECTED')
                        ...
                        if self.total_scans % 10 == 0:
                            self.get_logger().warn(
                                f'ICP P2Plane correction too large: {correction_dist:.3f}m/{np.rad2deg(correction_angle):.1f}°'
                            )
                else:
                    ...
                    self.consecutive_failures += 1
                    if self.total_scans % 10 == 0:
                        self.get_logger().warn(f'ICP P2Plane failed: success={success}, error={error:.4f}')
                
                # ========================================
                # Process Point-to-Point ICP (comparison path)
                # ========================================
                # Apply same delta from EKF to point-to-point pose
                predicted_p2point_pose = self.icp_p2point_pose + delta_ekf
                predicted_p2point_pose[2] = wrap(predicted_p2point_pose[2])
                
                # Validate point-to-point ICP result
                if success_p2point and error_p2point < 0.15:
                    if correction_dist_p2point < 0.15 and correction_angle_p2point < np.deg2rad(20):
                        candidate_pose_p2point = np.array([x_p2point, y_p2point, theta_p2point])
                        # Use same jump detection threshold
                        delta_p2point = candidate_pose_p2point - self.icp_p2point_pose
                        delta_dist_p2point = np.sqrt(delta_p2point[0]**2 + delta_p2point[1]**2)
                        if delta_dist_p2point < self.jump_threshold_dist:
                            self.icp_p2point_pose = candidate_pose_p2point
                        else:
                            self.icp_p2point_pose = predicted_p2point_pose
                    else:
                        self.icp_p2point_pose = predicted_p2point_pose
                else:
                    self.icp_p2point_pose = predicted_p2point_pose
        ...
        # Publish Point-to-Plane ICP path (main)
        self.publish_path(self.icp_path_pub, self.icp_path_msg, 
                         self.icp_pose[0], self.icp_pose[1], self.icp_pose[2], now)
        
        # Publish Point-to-Point ICP path (comparison)
        self.publish_path(self.icp_p2point_path_pub, self.icp_p2point_path_msg,
                         self.icp_p2point_pose[0], self.icp_p2point_pose[1], self.icp_p2point_pose[2], now)
        
        # Publish EKF path for comparison
    ...
    def timer_callback(self):
        """
        Periodic EKF update using wheel odometry and IMU.
        Runs at 20Hz (50ms intervals) to provide continuous pose estimation.
        """
        # Wait for IMU calibration to complete
        if not self.imu_calibrated:
            return
        
        # Need at least 2 wheel encoders
        ...
        if self.last_joint_pos is None or self.last_time is None:
            self.last_joint_pos = list(self.joint_pos)
        ...
        # Compute wheel displacements (radians * radius = meters)
        d_l = (self.joint_pos[0] - self.last_joint_pos[0]) * self.R
        d_r = (self.joint_pos[1] - self.last_joint_pos[1]) * self.R
        
        # Compute control input (differential drive kinematics)
        v = ((d_l + d_r) / 2.0) / dt  # Linear velocity (m/s)
        omega = ((d_r - d_l) / self.L) / dt  # Angular velocity (rad/s)
        
        # Control input vector
        u = np.array([[v], [omega]])
        
        # IMU yaw measurement (calibrated)
        z = np.array([[self.calibrated_yaw]])
        
        # Run EKF estimation (predict + update)
        self.ekf.estimate(u, z, dt)
        
        # Get estimated pose and wrap yaw angle
        x, y, theta = self.ekf.get_state()
        self.ekf_pose = np.array([x, y, wrap(theta)])
